...
Thread::Thread(char* threadName)
{
    ...
    status = JUST_CREATED;
    this->priority = 10;
    if(globalThreadManager->AddNewThreadtoList(this) == false)
    {this->~Thread();}
#ifdef USER_PROGRAM
    space = NULL;
#endif
...
Thread::Sleep ()
{
    ...
    //printf("current thread %s gone sleep , next thread %s is going running\n",currentThread->getName(),nextThread->getName());
