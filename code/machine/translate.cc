...
// from the virtual address
	...
	if (tlb == NULL) // => page table => vpn is index into table
	{
		...
		else
		{
			...
			bool flag =FALSE;
			for(int j = 0 ; j < 4 ; j++ )
			if( pageTable[j].virtualPage == vpn||pageTable[j].valid == TRUE)
				{entry = &pageTable[vpn];flag = TRUE;}
			if(flag == FALSE)
			{return PageFaultException;}
		}
else 
	...
	*/
	...
	if( machine->pageTable[j].virtualPage == vpn&&machine->pageTable[j].valid == TRUE&&machine->pageTable[j].thread==currentThread->getPid())
	{
		DEBUG('a',"Find a pagetable [%d]\n" ,j);
		entry = &machine->pageTable[j];
		flag = TRUE;
	}
	if(flag == FALSE)
	{
